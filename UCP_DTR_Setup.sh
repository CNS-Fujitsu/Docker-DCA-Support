...
docker container run --rm -it --name ucp -v /var/run/docker.sock:/var/run/docker.sock docker/ucp install --host-address x.x.x.x --interactive
...
# Deploy DTR on the manager using the manager
